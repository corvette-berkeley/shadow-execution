...
cl::opt<string> FileName("file", cl::value_desc("filename"), cl::desc("File names and indexes"),
						 cl::init("metadata.txt"));

cl::opt<string>
IncludedFunctions("includedFunctions", cl::value_desc("filename"),
				  cl::desc("List of functions to be instrumented; if skipped all functions are instrumented"),
				  cl::init("included.txt"));
...
struct MonitorPass : public FunctionPass {
		...
		for (Function::iterator e = F.end(); BB != e; ++BB) {
			...
			for (BasicBlock::iterator itr = BB->begin(), end = BB->end(); itr != end; ++itr) {
				...
				if (isFirstInstruction) {
					...
					Constant* frameSize = ConstantInt::get(Type::getInt32Ty(instrumentation->M_->getContext()),
				...
				if (isFirstBlockInstruction) {
					...
					Constant* blockId = ConstantInt::get(Type::getInt32Ty(instrumentation->M_->getContext()),
	...
	bool doInitialization(Module& M) {
		...
		for (int i = 0; i < length; i++) {
			...
			call = instrumenter->CALL_INSTR("llvm_push_string", instrumenter->VOID_FUNC_TYPE(argTypes), args);
		...
		Instruction* call =
			instrumenter->CALL_INSTR("llvm_create_global_symbol_table", instrumenter->VOID_FUNC_TYPE(argTypes), args);
		...
		for (Module::global_iterator i = M.global_begin(), e = M.global_end(); i != e; i++) {
			...
			if (!aryOfPrim) {
				...
				Instruction* call =
					instrumenter->CALL_INSTR("llvm_create_global", instrumenter->VOID_FUNC_TYPE(argTypes), args);
				instrs.push_back(call);
			}
...
template <class T> class RegisterInstrumenter {
public:
...
#define REGISTER_INSTRUMENTER(T, N) static RegisterInstrumenter<T> T##_INSTANCE(N);
...
static RegisterPass<MonitorPass> Z("instrument", "MonitorPass");
