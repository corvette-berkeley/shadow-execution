...
void BlameAnalysis::setShadowObject(SCOPE scope, int64_t inx, BlameTreeShadowObject<HIGHPRECISION>* shadowObject) {
  IValue *iv = NULL;
...
BlameTreeShadowObject<HIGHPRECISION>* BlameAnalysis::getShadowObject(SCOPE scope, int64_t inx) {
  IValue *iv = NULL;
...
HIGHPRECISION BlameAnalysis::getShadowValue(SCOPE scope, int64_t inx, PRECISION precision) {
...
void BlameAnalysis::post_lib_call(IID iid UNUSED, bool nounwind UNUSED, int pc,
  ...
  BlameTreeShadowObject<HIGHPRECISION> *shadow = getShadowObject(argScope, 
                                                                 argValueOrIndex
                                                                 );
  ...
  if (!shadow) {
    ...
    for (p = PRECISION(BITS_FLOAT+1); p < BITS_DOUBLE; p = PRECISION(p+1)) {
      values[p] = BlameTreeUtilities::clearBits((HIGHPRECISION) arg, 
                                      52 - BlameTreeUtilities::exactBits(p));
    }

    shadow = new BlameTreeShadowObject<HIGHPRECISION>(0, pc, 0, dpc, 
                                                      CONSTANT_INTR, BINOP_INVALID, 
                                                      "NONE", values);
  ...
  BlameTreeShadowObject<HIGHPRECISION> *resultShadow = new
    BlameTreeShadowObject<HIGHPRECISION>(0, pc, 0, dpc, 
                                        CALL_INTR, BINOP_INVALID, 
                                        func, values); 
...
void BlameAnalysis::post_call_sin(IID iid UNUSED, bool nounwind UNUSED, 
...
void BlameAnalysis::post_call_cos(IID iid UNUSED, bool nounwind UNUSED, 
                              int pc, KIND type UNUSED, int inx, 
                              SCOPE argScope, int64_t argValueOrIndex) {
  post_lib_call(iid, nounwind, pc, type, inx, argScope, argValueOrIndex, "cos");
}

void BlameAnalysis::post_call_acos(IID iid UNUSED, bool nounwind UNUSED, 
                              int pc, KIND type UNUSED, int inx, 
                              SCOPE argScope, int64_t argValueOrIndex) {
  post_lib_call(iid, nounwind, pc, type, inx, argScope, argValueOrIndex, "acos");
}

void BlameAnalysis::post_call_sqrt(IID iid UNUSED, bool nounwind UNUSED, 
                              int pc, KIND type UNUSED, int inx, 
                              SCOPE argScope, int64_t argValueOrIndex) {
  post_lib_call(iid, nounwind, pc, type, inx, argScope, argValueOrIndex, "sqrt");
}

void BlameAnalysis::post_call_fabs(IID iid UNUSED, bool nounwind UNUSED, 
                              int pc, KIND type UNUSED, int inx, 
                              SCOPE argScope, int64_t argValueOrIndex) {
  post_lib_call(iid, nounwind, pc, type, inx, argScope, argValueOrIndex, "fabs");
}

void BlameAnalysis::post_call_log(IID iid UNUSED, bool nounwind UNUSED, 
                              int pc, KIND type UNUSED, int inx, 
                              SCOPE argScope, int64_t argValueOrIndex) {
...
void BlameAnalysis::post_fbinop(SCOPE lScope, SCOPE rScope, 
                            int64_t lValue, int64_t rValue, KIND type, 
                            int file, int line, int col, int inx UNUSED, BINOP op) {
  ...
  if (!s1) {
    ...
    for (p = PRECISION(BITS_FLOAT+1); p < BITS_DOUBLE; p = PRECISION(p+1)) {
      values[p] = BlameTreeUtilities::clearBits((HIGHPRECISION) v1, 
                                      52 - BlameTreeUtilities::exactBits(p));
    }

    s1 = new BlameTreeShadowObject<HIGHPRECISION>(file, line, col, dpc, 
                                                  CONSTANT_INTR, BINOP_INVALID, 
                                                  "NONE", values);
  ...
  if (!s2) {
    ...
    s2 = new BlameTreeShadowObject<HIGHPRECISION>(file, line, col, 
                                                  dpc, CONSTANT_INTR, 
  ...
  BlameTreeShadowObject<HIGHPRECISION> *resultShadow = 
    new BlameTreeShadowObject<HIGHPRECISION>(file, line, col, dpc, 
...
void BlameAnalysis::post_analysis() {
  ...
  cout << "\tDesired precision: " <<
    BlameTreeUtilities::precisionToString(rootNode.getPrecision()) << endl;
  ...
  cout << "Construction time: " <<
    double(endTime-startTime)/double(CLOCKS_PER_SEC) << " seconds." << endl;
