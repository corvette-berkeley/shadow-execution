...
class BlameAnalysis : public InterpreterObserver {
  ...
  public:
    ...
    virtual void post_call_sin(IID iid, bool nounwind, int pc, KIND type, int
        inx, SCOPE argScope, int64_t argValueOrIndex);

    virtual void post_call_acos(IID iid, bool nounwind, int pc, KIND type, int
        inx, SCOPE argScope, int64_t argValueOrIndex);

    virtual void post_call_cos(IID iid, bool nounwind, int pc, KIND type, int
        inx, SCOPE argScope, int64_t argValueOrIndex);

    virtual void post_call_sqrt(IID iid, bool nounwind, int pc, KIND type, int
        inx, SCOPE argScope, int64_t argValueOrIndex);

    virtual void post_call_fabs(IID iid, bool nounwind, int pc, KIND type, int
        inx, SCOPE argScope, int64_t argValueOrIndex);

    virtual void post_call_log(IID iid, bool nounwind, int pc, KIND type, int
        inx, SCOPE argScope, int64_t argValueOrIndex);

    virtual void post_call_floor(IID iid, bool nounwind, int pc, KIND type, int
        inx, SCOPE argScope, int64_t argValueOrIndex);
