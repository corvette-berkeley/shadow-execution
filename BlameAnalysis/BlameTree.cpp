...
const BlameNode& BlameTree::constructFuncBlameNode(const BlameTreeShadowObject<HIGHPRECISION>& left,
		PRECISION precision,
	...
	//
	BlameNode node(dpc, pc, col, file, false, precision, {}, {});
...
const BlameNode& BlameTree::constructBlameNode(const BlameTreeShadowObject<HIGHPRECISION>& left, PRECISION precision,
...
// is bad...
const BlameNode& BlameTree::constructBlameGraph(const vector<vector<BlameTreeShadowObject<HIGHPRECISION>>>& trace) {
...
struct location {
	...
	int pc;
	int col;
	location(string f, int p, int c) : file(f), pc(p), col(c) {}
...
void BlameTree::printResult() const {
	...
	while (!workList.empty()) {
		...
		//
		if (result.find(location(file, pc, col)) == result.end()) {
			result[location(file, pc, col)] = highlighting(highlight, edgeHighlight);
		} else {
			location loc = location(file, pc, col);
