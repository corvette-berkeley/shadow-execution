...
void BlameAnalysis::post_lib_call(IID iid, IID argIID, SCOPE argScope, int64_t argVal, KIND type UNUSED, int inx,
								  MATHFUNC func) {
...
void BlameAnalysis::computeBlameSummary(const BlameShadowObject& bso, const BlameShadowObject& argbso, MATHFUNC func) {
	...
	if (blameSummary.find(id) == blameSummary.end()) {
		...
		new_roots[BITS_FLOAT] = new BlameNode(id, BITS_FLOAT, false, false, {{nullptr}});
		safe_assert(new_roots[BITS_FLOAT]->getSize() == 0);
		for (PRECISION p = PRECISION(BITS_FLOAT + 1); p < PRECISION_NO; p = PRECISION(p + 1)) {
			new_roots[p] = new BlameNode(id, p, false, false, {{blameSummary[argbso.id][BITS_FLOAT], nullptr}});
...
void BlameAnalysis::computeBlameSummary(const BlameShadowObject& bso, const BlameShadowObject& lbso,
	...
	if (blameSummary.find(id) == blameSummary.end()) {
		...
		for (PRECISION p = PRECISION(BITS_FLOAT + 1); p < PRECISION_NO; p = PRECISION(p + 1)) {
			new_roots[p] =
			new BlameNode(id, p, false, false, {{blameSummary[lbso.id][BITS_FLOAT], blameSummary[rbso.id][BITS_FLOAT]}});
	...
	// current blame summary.
	for (PRECISION p = PRECISION(BITS_FLOAT + 1); p < PRECISION_NO; p = PRECISION(p + 1)) {
		const BlameNode& blameInfo = computeBlameInformation(bso, lbso, rbso, op, p);
		mergeBlame(roots[p], blameInfo);
	}
}

const BlameNode BlameAnalysis::computeBlameInformation(const BlameShadowObject& bso, const BlameShadowObject& argbso,
		MATHFUNC func, PRECISION p) {
	HIGHPRECISION val = clearBits(bso.highValue, DOUBLE_MANTISSA_LENGTH - PRECISION_BITS[p]);
	...
	auto bn = BlameNode(bso.id, p, requireHigherPrecision, requireHigherPrecisionOperator, {{argBlameNode, nullptr}});
	safe_assert(bn.getSize() == 1);
	return bn;
}

const BlameNode BlameAnalysis::computeBlameInformation(const BlameShadowObject& bso, const BlameShadowObject& lbso,
	...
	auto bn = BlameNode(bso.id, p, requireHigherPrecision, requireHigherPrecisionOperator, {{lBlameNode, rBlameNode}});
...
inline bool BlameAnalysis::canBlame(HIGHPRECISION result, HIGHPRECISION lop, HIGHPRECISION rop, BINOP op, PRECISION p) {
	return equalWithinPrecision(
...
void BlameAnalysis::mergeBlame(BlameNode* summary, const BlameNode& blame) {
	...
	summary->requireHigherPrecision = summary->requireHigherPrecision || blame.requireHigherPrecision;
	summary->requireHigherPrecisionOperator =
		summary->requireHigherPrecisionOperator || blame.requireHigherPrecisionOperator;
}
...
void BlameAnalysis::initSummaryIfNotExist(IID id) {
	if (blameSummary.find(id) == blameSummary.end()) {
		...
		for (PRECISION p = BITS_FLOAT; p < PRECISION_NO; p = PRECISION(p + 1)) {
			blames[p] = new BlameNode(id, p, false, false, {{nullptr}});
...
void BlameAnalysis::post_call_sin(IID iid, IID argIID, SCOPE argScope, int64_t argVal, KIND type, int inx) {
	post_lib_call(iid, argIID, argScope, argVal, type, inx, SIN);
}

void BlameAnalysis::post_call_exp(IID iid, IID argIID, SCOPE argScope, int64_t argVal, KIND type, int inx) {
	post_lib_call(iid, argIID, argScope, argVal, type, inx, EXP);
}

void BlameAnalysis::post_call_cos(IID iid, IID argIID, SCOPE argScope, int64_t argVal, KIND type, int inx) {
	post_lib_call(iid, argIID, argScope, argVal, type, inx, COS);
}

void BlameAnalysis::post_call_acos(IID iid, IID argIID, SCOPE argScope, int64_t argVal, KIND type, int inx) {
	post_lib_call(iid, argIID, argScope, argVal, type, inx, ACOS);
}

void BlameAnalysis::post_call_sqrt(IID iid, IID argIID, SCOPE argScope, int64_t argVal, KIND type, int inx) {
	post_lib_call(iid, argIID, argScope, argVal, type, inx, SQRT);
}

void BlameAnalysis::post_call_fabs(IID iid, IID argIID, SCOPE argScope, int64_t argVal, KIND type, int inx) {
	post_lib_call(iid, argIID, argScope, argVal, type, inx, FABS);
}

void BlameAnalysis::post_call_log(IID iid, IID argIID, SCOPE argScope, int64_t argVal, KIND type, int inx) {
	post_lib_call(iid, argIID, argScope, argVal, type, inx, LOG);
}

void BlameAnalysis::post_call_floor(IID iid, IID argIID, SCOPE argScope, int64_t argVal, KIND type, int inx) {
	...
	while (!workList.empty()) {
		...
		if (node->requireHigherPrecision || node->requireHigherPrecisionOperator) {
			printf("Function %s, Line %d, Column %d, HigherPrecision: %d, "
				   ...
				   dbg.file, dbg.line, dbg.column, node->requireHigherPrecision, node->requireHigherPrecisionOperator);
		}
	}
