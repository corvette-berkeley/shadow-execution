...
int
gsl_sum_levin_u_accel (const float *array, const size_t array_size,
                       gsl_sum_levin_u_workspace * w, 
                       float *sum_accel, float *abserr)
{
                        ...
                        double *sum_accel, float *abserr)
{
  ...
  if (size == 0)
  ...
  else
    {
      ...
      float noise_n = 0.0; float noise_nm1 = 0.0;
      float trunc_n = 0.0; float trunc_nm1 = 0.0;
      float actual_trunc_n = 0.0, actual_trunc_nm1 = 0.0;
      double result_n = 0.0; float result_nm1 = 0.0;
      float variance = 0;
      size_t n;
      unsigned int i;
      int better = 0;
      ...
      float least_trunc_noise = GSL_DBL_MAX;
      float least_trunc_result;
      ...
      for (i = 0; i < n; i++)
        {
          float dn = w->dsum[i] * GSL_MACH_EPS * array[i];
          variance += dn * dn;
        }
