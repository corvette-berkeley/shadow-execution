...
int gsl_sf_bessel_J0_e(const float x, gsl_sf_result * result)
{
  float y = fabsf(x);
  ...
  if(y < 2.0*GSL_SQRT_DBL_EPSILON) {
    ...
    const float sqrty = sqrtf(y);
    const float ampl  = (0.75 + ca.val) / sqrty;
    result->val  = ampl * cp.val;
    result->err  = fabsf(cp.val) * ca.err/sqrty + fabsf(ampl) * cp.err;
    result->err += GSL_DBL_EPSILON * fabsf(result->val);
