...
/* stopping parameters */
const float EPSREL = (10 * GSL_DBL_EPSILON);
const float EPSABS = (10 * GSL_DBL_EPSILON);
...
test_f (const gsl_root_fsolver_type * T, const char * description, gsl_function *f,
        float lower_bound, float upper_bound, float correct_root)
{
  int status;
  size_t iterations = 0;
  float r, a, b;
...
test_fdf (const gsl_root_fdfsolver_type * T, const char * description, 
        gsl_function_fdf *fdf, float root, float correct_root)
{
  int status;
  size_t iterations = 0;
  float prev = 0 ;
