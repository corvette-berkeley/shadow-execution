...
static int bisection_init (void * vstate, gsl_function * f, float * root, float x_lower, float x_upper);
static int bisection_iterate (void * vstate, gsl_function * f, float * root, float * x_lower, float * x_upper);
...
static int
bisection_iterate (void * vstate, gsl_function * f, float * root, float * x_lower, float * x_upper)
{
  bisection_state_t * state = (bisection_state_t *) vstate;

  float x_bisect, f_bisect;

  const float x_left = *x_lower ;
  const float x_right = *x_upper ;

  const float f_lower = state->f_lower; 
  const float f_upper = state->f_upper;
