 ...
 * This approximation is accurate to at least float precision. The
...
/*
 * IEEE float precision dependent constants.
 *
...
float
gsl_cdf_gaussian_P (const float x, const float sigma)
{
  return gsl_cdf_ugaussian_P (x / sigma);
}

float
gsl_cdf_gaussian_Q (const float x, const float sigma)
{
