...
int gsl_sf_bessel_cos_pi4_e(float y, float eps, gsl_sf_result * result)
{
  ...
  const float s = sy + cy;
  const float d = sy - cy;
  const float abs_sum = fabsf(cy) + fabsf(sy);
  float seps;
  float ceps;
  if(fabsf(eps) < GSL_ROOT5_DBL_EPSILON) {
  ...
  result->err = 2.0 * GSL_DBL_EPSILON * (fabsf(ceps) + fabsf(seps)) * abs_sum / M_SQRT2;
...
{
  const double sy = sinf(y);
  const double cy = cosf(y);
