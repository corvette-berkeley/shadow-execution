...
static int brent_init (void * vstate, gsl_function * f, float * root, float x_lower, float x_upper);
static int brent_iterate (void * vstate, gsl_function * f, float * root, float * x_lower, float * x_upper);


static int
brent_init (void * vstate, gsl_function * f, float * root, float x_lower, float x_upper)
{
...
static int
brent_iterate (void * vstate, gsl_function * f, float * root, float * x_lower, float * x_upper)
{
  brent_state_t * state = (brent_state_t *) vstate;
  ...
  float fa = state->fa, fb = state->fb, fc = state->fc;
  float d = state->d, e = state->e;
  ...
  if (fabs (e) < tol || fabs (fa) <= fabs (fb))
    ...
    {
      float p, q, r;   /* use inverse cubic interpolation */
      float s = fb / fa;
