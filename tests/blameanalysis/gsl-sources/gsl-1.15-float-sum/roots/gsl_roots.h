...
typedef struct {
	const char* name;
	size_t size;
	int (*set)(void* state, gsl_function* f, float* root, float x_lower, float x_upper);
	int (*iterate)(void* state, gsl_function* f, float* root, float* x_lower, float* x_upper);
...
typedef struct {
	const char* name;
	size_t size;
	int (*set)(void* state, gsl_function_fdf* f, float* root);
	int (*iterate)(void* state, gsl_function_fdf* f, float* root);
...
int gsl_root_fsolver_set(gsl_root_fsolver* s, gsl_function* f, float x_lower, float x_upper);

int gsl_root_fsolver_iterate(gsl_root_fsolver* s);

const char* gsl_root_fsolver_name(const gsl_root_fsolver* s);
float gsl_root_fsolver_root(const gsl_root_fsolver* s);
float gsl_root_fsolver_x_lower(const gsl_root_fsolver* s);
float gsl_root_fsolver_x_upper(const gsl_root_fsolver* s);


gsl_root_fdfsolver* gsl_root_fdfsolver_alloc(const gsl_root_fdfsolver_type* T);

int gsl_root_fdfsolver_set(gsl_root_fdfsolver* s, gsl_function_fdf* fdf, float root);
...
float gsl_root_fdfsolver_root(const gsl_root_fdfsolver* s);

int gsl_root_test_interval(float x_lower, float x_upper, float epsabs, float epsrel);

int gsl_root_test_residual(float f, float epsabs);

int gsl_root_test_delta(float x1, float x0, float epsabs, float epsrel);
