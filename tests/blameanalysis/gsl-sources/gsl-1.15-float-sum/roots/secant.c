...
static int secant_init (void * vstate, gsl_function_fdf * fdf, float * root);
static int secant_iterate (void * vstate, gsl_function_fdf * fdf, float * root);

static int
secant_init (void * vstate, gsl_function_fdf * fdf, float * root)
{
...
static int
secant_iterate (void * vstate, gsl_function_fdf * fdf, float * root)
{
  secant_state_t * state = (secant_state_t *) vstate;
  
  const float x = *root ;
  const float f = state->f;
  const float df = state->df;

  float x_new, f_new, df_new;
