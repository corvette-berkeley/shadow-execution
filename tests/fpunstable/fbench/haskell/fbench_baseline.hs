...
--            _ <- de `deepseq` return (0 :: Int)
            let de = dev        -- Null benchmark for comparison
            let rn = fst(nrand)
            _ <- rn `deepseq` return (0 :: Int)
...
{-                          Main program
    ...
    main = do
        ...
        let _ = validate_results results_expected report
        -- We don't validate results in baseline since, obviously, they
        -- won't validate since we didn't do the ray trace.
