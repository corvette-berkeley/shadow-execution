...
{-  The report_validation_results function reports the results of
    ...
    report_validation_results (False, matches) expected received =
        ...
        list_discrepancies :: [Bool] -> [String] -> [String] -> String -> Int -> String
...
{-  The run_benchmark function runs the raytrace and analysis the
    ...
    run is returned.  We pass the last DesignEvaluation into the
    next recursion not because it's needed, but because it gives
    us a pretext to force evaluation.  -}

    run_benchmark :: Int -> DesignEvaluation -> DesignEvaluation

    run_benchmark 1 _ =
            evaluate_design wyldLens wyldClearAperture

    run_benchmark n _ = do
            let x = evaluate_design wyldLens wyldClearAperture
            run_benchmark (n - 1) x
...
{-  The time_benchmark function runs the ray trace and evaluation
    ...
    This yields IO types both because access to the current time
    corrupts the purity of the function, but also to permit us to force
    evaluation to perform a legitimate benchmark and not be optimised
    into triviality.  -}
    ...
    time_benchmark n = do
        ...
        let de = run_benchmark n (DesignEvaluation
                                 (RayResult 0 0)
                                 (RayResult 0 0)
                                 0 0 0 0 0 0)
        end <- getCurrentTime
